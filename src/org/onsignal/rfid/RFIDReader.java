...
public class RFIDReader extends Thread
{
    ...
    /**
     * Opens the port and starts the reader thread. If the reader was already running this function returns false, otherwise true.
     */
    public boolean open() throws PortInUseException 
    {
		...
		try {
		    ...
		    // If any of this initialization stuff fails, we're in trouble.
    ...
    private void sendCommand(int cmd, int[] data) throws IOException 
    {
		...
		int checksum = data.length+1 + cmd;
		...
		out.write(command, 0, 5+data.length);
    }
